...
**Branch:** `greyzxc/agentic-security-test-coverage-cb45`  
**Trigger context:** PR #23 (`coderabbitai/autofix/a17c857` on `feat/tcg-arena-rulesets`) — CodeRabbit auto-fixes atop dynamic playmats, rulesets, and decoupled `scoreFormulaRegistry`  
**Date:** 2026-06-08
...
| `scoreFormulaRegistry` | Wrong formula per playmat after CodeRabbit refactor | `rulesets.test.js` + `battleEngine` integration |
...
|----------------|--------|------|-------|
| `rulesets.test.js` | High — registry refactor removed inline `calculateScore` | Low | Vitest ~ms |
...
| `src/domain/rulesets.test.js` | **New** — schema + `scoreFormulaRegistry` |

...
- `e2e/*.spec.ts` — Playwright specs (`app.spec.ts` core flows, `arena-security.spec.ts` ruleset/clash/boundary, `default-deck-agent-ux.spec.ts` canonical deck + migration)
- `src/test/fixtures/AetherTestFixtures.js` — constructor-based unit test fixtures
- `src/test/fixtures/DefaultDeckHarness.js` — shipped `default_deck.json` invariants
- `src/domain/defaultDeck.test.js` — canonical deck schema + forge id boundary

...
describe('usePersistedDeck — hostile edge / persistence boundary', () => {
  ...
  it('rejects agent-injected activeDeckId pointing at missing deck without crashing', () => {
    const orphanState = JSON.stringify({
      activeDeckId: 'ghost-deck',
      decks: [
        {
          id: 'default',
          name: 'Real Deck',
    ...
    const storage = createMemoryDeckStorage({ 'aether-decks': orphanState });
    const { result } = renderHook(() =>
      usePersistedDeck(storage, null, fallbackDeck),
    );

    expect(result.current.decks).toHaveLength(1);
    expect(result.current.activeDeckId).toBe('ghost-deck');
    expect(result.current.deck).toEqual(fallbackDeck);
  });
});
